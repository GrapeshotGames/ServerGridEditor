...
namespace ServerGridEditor
{
	 public class Island
	 {
		  ...
		  public Island(string name, float x, float y, string imagePath, int landscapeMaterialOverride
				...
				bool useLevelBoundsForTreasures,
				bool prioritizeVolumesForTreasures,
				bool isControlPoint,
				bool isControlPointAllowCapture,
				string IslandTreasureBottleSupplyCrateOverrides,
				List<string> extraSublevels,
				int islandPoints,
				float sPlayerSpawnPointX,
				float sPlayerSpawnPointY,
				float sPlayerSpawnPointZ,
				float theMaxIslandClaimFlagZ)
		  {
				this.name = name;
				this.x = x;
				this.y = y;
